...
const Label = styled.div<{ $labelX: number; $labelY: number; $isError?: boolean }>`
  ...
  border-radius: 360px;
  border: 1px solid ${({ $isError, theme }) => ($isError ? (theme as any).colors.dark_red : (theme as any).colors.border)};
  color: ${({ $isError, theme }) => ($isError ? (theme as any).text.error : (theme as any).text.light_grey)};
  font-family: ${({ theme }) => (theme as any).font_family.secondary};
  font-size: 10px;
  font-weight: 400;
