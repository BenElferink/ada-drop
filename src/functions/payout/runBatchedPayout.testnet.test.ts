...
import { confirmTxOnBlockfrost, createFundedPreprodWallet, runBatchedPayout } from './index'
...
if (REQUIRE_SECRETS && !hasSecrets) {
  throw new Error('REQUIRE_TESTNET_SECRETS=1 but BLOCKFROST_API_KEY_PREPROD is missing')
}
...
describe.skipIf(!hasSecrets)('runBatchedPayout on Cardano preprod', () => {
  it('funds a throwaway wallet from the faucet, then completes ≥2 chained batched TXs', async () => {
    ...
    console.log(`Funded throwaway preprod wallet ${address} with ${lovelace} lovelace`)
    ...
    const api = new BlockFrostAPI({ projectId: BLOCKFROST_KEY as string, network: 'preprod' })
    const utxos = await api.txsUtxos(firstHash)
    const payoutOutputs = utxos.outputs.filter((o) =>
      recipients.some(
        (r) => r.address === o.address && o.amount.some((a) => a.unit === 'lovelace' && a.quantity === '1500000')
      )
    )
