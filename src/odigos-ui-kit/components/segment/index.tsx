...
const Segment: FC<SegmentProps> = ({ options = [], selected, setSelected }) => {
  ...
  return (
    <Container $count={options.length}>
      {options.map(({ icon: Icon, label, value, selectedBgColor, selectedTextColor }, idx) => {
        const isSelected = selected === value;

        return (
          <Button
            key={`segment-${label || idx}-${value}`}
          ...
          >
            {Icon && <Icon />}
            {label && (
              <Text size={12} family='secondary' decoration='underline' color={isSelected ? selectedTextColor : undefined}>
                {label}
              </Text>
            )}
          </Button>
